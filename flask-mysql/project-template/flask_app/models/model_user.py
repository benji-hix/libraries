...
#* ------------------------------ validate login ------------------------------ #
    ...
    def validate_login(form):
        ...
        if len(found_user) < 1:
            ...
            is_valid = False
            return is_valid 
            #! password only checked if user is found 
...
#`` -------------------------- show one user with CHANGE ------------------------- #
    @classmethod
    def user_with_(cls, pk):#*CHANGE
        query = """SELECT * FROM users
                LEFT JOIN #*CHANGE 
                ON users.id = .user_id #*CHANGE
                WHERE users.id = %(id)s
        ...
        for row in results:
            ...
            user._list.append( model_.(_data) )#*CHANGE
